...
export async function POST(request: NextRequest) {
  ...
  const updated = await prisma.bankTransaction.update({
    where: { id: bankTransactionId },
    data: { recon_status: 'excluded', matched_payment_id: null, matched_invoice_id: null, matched_sales_invoice_id: null, notes: notes || null, matched_at: new Date(), matched_by: session.user.id },
  });
