...
export async function POST(request: NextRequest) {
  ...
  // Verify supplier belongs to this firm
  ...
  // Validate each allocation doesn't overpay the invoice
  ...
  // Create payment + allocations + receipt links in transaction
  const payment = await prisma.payment.create({
    data: {
      firm_id: firmId,
      supplier_id,
  ...
  // Recalculate each invoice's amount_paid
  for (const alloc of allocations) {
    await recalcInvoicePayment(alloc.invoice_id);
  }
