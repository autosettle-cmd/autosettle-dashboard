...
export default function ReconciliationWorkspacePage() {
  ...
  const doExclude = async () => {
    ...
    await fetch('/api/admin/bank-reconciliation/exclude', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
          ...
          {matchingTxn && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50 flex items-center justify-center" onClick={() => setMatchingTxn(null)}>
              <div className="bg-white rounded-lg shadow-xl p-6 w-[560px] max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={() => setMatchingTxn(null)} className="mt-4 w-full px-3 py-2 text-body-md text-[#434654] border border-gray-200 rounded-md hover:bg-gray-50">
                  Cancel
                </button>
              </div>
            </div>
              ...
              <div className="bg-white rounded-lg shadow-xl p-6 w-[420px]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-1.5 mb-4">
                  {[
                    ...
                    <label key={opt.value}
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-md border cursor-pointer transition-colors ${excludeReason === opt.value ? 'border-blue-300 bg-blue-50/50' : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50'}`}
                    >
                ...
                <div className="flex gap-2">
                  <button onClick={() => setExcludingTxn(null)} className="flex-1 px-3 py-2 text-body-md text-[#434654] border border-gray-200 rounded-md hover:bg-gray-50">Cancel</button>
