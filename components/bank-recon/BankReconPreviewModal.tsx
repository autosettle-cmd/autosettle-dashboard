          ...
          <div className="w-1/2 flex flex-col min-h-0">
            ...
            <div className="flex-1 min-h-0 bg-[var(--surface-low)] relative">
              {(() => {
                ...
                // Check if matched PV/OR without document — offer PDF generation
                ...
                if (hasMatches && (canGeneratePV || canGenerateOR)) {
                  ...
                  const invoiceId = inv?.id ?? si?.id;
                  return (
                    <div className="w-full h-full flex flex-col items-center justify-center gap-3 p-8">
                      <p className="text-sm text-[var(--text-muted)]">No document attached</p>
                      <button
                        onClick={async () => {
                          const blob = generateVoucherPdf({
                            type: canGeneratePV ? 'PV' : 'OR',
                          ...
                          if (!invoiceId) return;
                          const file = new File([blob], `${(inv?.invoice_number ?? si?.invoice_number ?? 'voucher').replace(/[^a-zA-Z0-9-]/g, '_')}.pdf`, { type: 'application/pdf' });
                          const fd = new FormData();
                          fd.append('file', file);
                          fd.append('generated', 'true');
                          const res = await fetch(`/api/invoices/${invoiceId}/attach`, { method: 'PATCH', body: fd });
                          if (res.ok) onRefresh?.();
                        }}
                      ...
                      >
                        ...
                        <p className="text-sm font-medium text-[var(--text-primary)]">Generate {canGeneratePV ? 'Payment Voucher' : 'Official Receipt'}</p>
                        <p className="text-xs text-[var(--text-muted)] mt-1">Creates PDF and attaches to record</p>
                      </button>
                    </div>
                  );
                }
