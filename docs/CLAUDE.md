...
| **Firm setup guard** | All upload/submit pages must check firm setup status before allowing uploads. If COA, GL defaults, category mappings, or fiscal year is missing, show amber warning banner with "Go to Setup" link and block the upload. Check via `GET /api/accountant/firms/{firmId}/setup-status`. Required steps: Firm Details, COA, GL Defaults (Trade Payables + Staff Claims), Category→GL Mapping, Fiscal Year. Add Admin is optional. |
